...
default(d1(X), pos(has_legal_capacity(X)), [], [pos(minor(X))]) :- citizen(X).
default(d2(X), pos(has_right_to_vote(X)), [neg(minor(X))], []) :- citizen(X).
...
defeated(D) :- exception(D, PositiveBody, NegativeBody),
               hold_by_default(D, PositiveBody),
	       fail_by_default(D, NegativeBody).
